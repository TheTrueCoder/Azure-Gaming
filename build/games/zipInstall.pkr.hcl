...
variable "game_url" {
  ...
  default = ""
  validation {
    condition = length(var.game_url) > 0
    error_message = "Please set the download url for the game."
  }
}
...
source "azure-arm" "main" {
  ...
  custom_managed_image_name         = "Base-Windows"
  custom_managed_image_resource_group_name  = "AUS_CloudGaming"
  os_type                           = "Windows"
  
  managed_image_name                = "CavesRD-AusCRB"
...
build {
  ...
  provisioner "powershell" {
    inline = ["Invoke-WebRequest -Uri ${var.game_url} -OutFile ~\game.zip"]
  }

  # Process game files
  provisioner "powershell" {
    script = "install_zip.ps1"
  }
